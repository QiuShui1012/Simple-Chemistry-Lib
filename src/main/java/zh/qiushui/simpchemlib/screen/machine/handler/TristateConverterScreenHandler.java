...
public class TristateConverterScreenHandler extends ScreenHandler {
    private final Inventory inventory;
    private final PropertyDelegate propertyDelegate;
    public final TristateConverterEntity blockEntity;
    public TristateConverterScreenHandler(int syncId, PlayerInventory inventory, PacketByteBuf buf){
        this(syncId,inventory,inventory.player.getWorld().getBlockEntity(buf.readBlockPos()),
                new ArrayPropertyDelegate(17));
    }

    public TristateConverterScreenHandler(int syncId, PlayerInventory playerInventory, BlockEntity blockEntity, PropertyDelegate arrayPropertyDelegate) {
        super(MachineRegistry.ELEMENT_EXTRACTOR_SCREEN_HANDLER,syncId);
        checkSize((Inventory) blockEntity,17);
        this.inventory = (Inventory) blockEntity;
        inventory.onOpen(playerInventory.player);
        this.propertyDelegate = arrayPropertyDelegate;
        this.blockEntity = (TristateConverterEntity) blockEntity;
